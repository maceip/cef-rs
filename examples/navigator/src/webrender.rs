//! CEF client, render handler, and process wiring for the navigator (OSR) host.
//!
//! This crate is intended to grow toward the same command surface as
//! [agent-browser](https://github.com/vercel-labs/agent-browser) (see its README “Commands”),
//! backed by the maceip/cef journal-branch build and agent-oriented C APIs.
    ...
    impl App {
        ...
        fn browser_process_handler(&self) -> Option<cef::BrowserProcessHandler> {
            Some(BrowserProcessHandlerBuilder::build(
                NavigatorBrowserProcessHandler::new(),
            ))
        }
    }
}
...
impl AppBuilder {
    pub(crate) fn build(app: NavigatorApp) -> cef::App {
        Self::new(app)
    }
}

#[derive(Clone)]
pub struct NavigatorBrowserProcessHandler {
    is_cef_ready: RefCell<bool>,
}

impl NavigatorBrowserProcessHandler {
    pub fn new() -> Self {
        Self {
...
wrap_browser_process_handler! {
    pub(crate) struct BrowserProcessHandlerBuilder {
        handler: NavigatorBrowserProcessHandler,
    }
...
impl BrowserProcessHandlerBuilder {
    pub(crate) fn build(handler: NavigatorBrowserProcessHandler) -> BrowserProcessHandler {
        Self::new(handler)
    }
}

#[derive(Clone)]
pub struct NavigatorRenderHandler {
...
wrap_render_handler! {
    pub struct RenderHandlerBuilder {
        handler: NavigatorRenderHandler,
    }
...
impl RenderHandlerBuilder {
    pub fn build(handler: NavigatorRenderHandler) -> RenderHandler {
        Self::new(handler)
    }
}
...
impl ClientBuilder {
    pub(crate) fn build(render_handler: NavigatorRenderHandler) -> Client {
        Self::new(RenderHandlerBuilder::build(render_handler))
    }
}

#[derive(Clone)]
pub struct NavigatorRequestContextHandler {}

wrap_request_context_handler! {
    pub(crate) struct RequestContextHandlerBuilder {
        handler: NavigatorRequestContextHandler,
    }
...
impl RequestContextHandlerBuilder {
    pub(crate) fn build(handler: NavigatorRequestContextHandler) -> RequestContextHandler {
        Self::new(handler)
    }
}
